...
export const OrderDispatchCenter: React.FC = () => {
  ...
  const columns: { status: OrderStatus; label: string; color: string }[] = [
    { status: 'pending', label: 'Pending Dispatch', color: 'border-amber-400' },
                    ...
                    <td className="p-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {order.status === 'pending' && (
                          <button
                            onClick={() => setAssigningOrder(order)}
                            className="px-2 py-1 bg-indigo-600 text-white rounded-md text-[11px] font-semibold"
                          >
                            Dispatch
                          </button>
                        )}
                        ...
                        {order.status === 'in_progress' && (
                          <button
                            ...
                            className="px-2 py-1 bg-slate-900 text-white rounded-md text-[11px] font-semibold"
                          >
                            Finish
                          </button>
                        )}
