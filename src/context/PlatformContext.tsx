...
export const PlatformProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [currentUser] = useState<CustomerUser>(CURRENT_CUSTOMER);
  ...
  const placeOrder = (booking: {
    ...
    const newOrder: Order = {
      ...
      discount: 10,
      totalAmount: Math.max(10, booking.service.price - 10),
      status: 'pending',
    ...
    addAuditLog('customer', currentUser.name, 'Order Placed', `Created order #${orderNum} for $${newOrder.totalAmount}.00`, orderNum);
